...
## 追記（2026-09-02）— §4 / §5 / §6 は実装した

この文書は最初「割り当てと調査結果だけ。C++ は触っていない」という
状態で書かれた。その後、§8 に挙がっていた変更のうち**受け口の側**を
実装した。**実機が無いという前提は何も変わっていない**（§0）。

実装したもの:

| 節 | 内容 | 場所 |
|---|---|---|
| §4 | **H パターンの絶対指定** `SetGearAbsolute` と `ZN6_Gear1..6` / `ZN6_GearNeutral` / `ZN6_GearReverse` / `ZN6_GearAbsolute` の受け口 | `ZN6VehicleActor.cpp` |
| §4 | **ニュートラルと後退を物理モデルに入れた**（Python 先行 → C++ 移植） | `Physics/drivetrain.py` / `Physics/vehicle.py` / `Physics/ZN6Components.*` / `Physics/ZN6Vehicle.*` |
| §4 | シーケンシャルの並びを **R → N → 1 … 6** にした | `ZN6VehicleActor.cpp` |
| §5 | **アナログ時は平滑化と速度絞りを切る**（`bAnalogInput`。`-ZN6Analog` で入れる。自動判定はしない） | `ZN6VehicleActor.cpp` |
| §6 | **メニューをパッドで操作・脱出できるようにした**。そのうえで `ZN6_Menu` をパッドへ割り当てた | `UI/SZN6Menu.cpp` / `DefaultInput.ini` |

実装していないもの（**できていないことをできたと書かない**）:

- **フォースフィードバック**（§7）。UE 側に出す口が無い。加えて
  ニューマチックトレール・キャスタトレール・ステアリングギア比が
  vehicle.json で `"unknown"` なので、**物理的に正しい反力は出せない**
- **RawInput / GameInput プラグインの有効化**。`bProcessRacingWheel` と
  `RacingWheelDeadzone` は `UPlatformSettings` で、`DefaultInput.ini` に
  書けるか確かめられていない（§3）
- **実機での確認**。DualSense も T300RS も TH8A も手元に無い。
  自動テスト（`ZN6.Input.*` 6 件）が見ているのは
  **「キーが来たあと、こちら側がそれをどう物理へ渡すか」だけ**である

なお、実装の途中で**テストを全部通ったまま起動画面が壊れていた**箇所が
1 つ出た。`ZN6_GearAbsolute` の軸は繋がっていないと毎フレーム 0 を出し、
それを「ニュートラルに入れろ」と読んでいたため、起動するたび段が N へ
落ちて車が 1 mm も動かなかった。**画面を撮って初めて分かった。**
今は `ZN6.Input.繋がっていないシフターの軸が段を奪わない` が見ている。
...
## 8. C++ 側に要る変更（当時の一覧。**受け口の側は上の追記で実装済み**）
